...
def calculates_results_stats(results_dic):
    ...
    n_dogs_img = sum(result[3] == 1 for result in results_dic.values())
    n_notdogs_img = sum(result[3] == 0 for result in results_dic.values())
    n_match = sum(result[2] == 1 for result in results_dic.values())
    n_correct_dogs = sum(result[3] == 1 and result[4] == 1 for result in results_dic.values())
    n_correct_notdogs = sum(result[3] == 0 and result[4] == 0 for result in results_dic.values())
    n_correct_breed = sum(result[2] == 1 and result[3] == 1 for result in results_dic.values())

    pct_match = (n_match / n_images) * 100 if n_images > 0 else 0
    pct_correct_dogs = (n_correct_dogs / n_dogs_img) * 100 if n_dogs_img > 0 else 0
    pct_correct_breed = (n_correct_breed / n_correct_dogs) * 100 if n_correct_dogs > 0 else 0
